...
void controls::computeMatricesFromInputs_spherical(GLFWwindow* window) {
	...
	*/
    ...
    if(L_pressed)
    {
        ...
        currentTime = glfwGetTime();
        deltaTime = float(currentTime - lastTime); // Get time difference
    }
    ...
    glm::vec3 direction = sphere_center - position;

    // Right vector (used for calculating the Up vector)
    ...
    if(R_pressed)
    {
        ...
        currentTime = glfwGetTime();
        deltaTime = float(currentTime - lastTime);
        //sphere_center = direction * deltaTime * speed;        // <<<<<<<<<<<<<<< Take direction & up into account
    }
